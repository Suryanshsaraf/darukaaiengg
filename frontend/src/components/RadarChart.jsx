...
export default function RadarChart({ profile, recommendations }) {
  const categories = [
    { label: "Soil Carbon Stock", key: "soc" },
    { label: "Water Infiltration", key: "water" },
    { label: "Mycorrhizal Fungi", key: "fungi" },
    { label: "Biodiversity Richness", key: "bio" },
    { label: "Ground Cover", key: "cover" },
    { label: "Microclimate Buffer", key: "micro" }
  ];
  ...
  return (
    <div className="bg-slate-800/80 border border-slate-700/80 rounded-xl p-4 flex flex-col items-center">
      <div className="w-full flex items-center justify-between mb-2">
        <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Multi-Metric Impact Polygon</span>
        <div className="flex items-center space-x-3 text-xs">
          ...
          <span className="flex items-center space-x-1 text-emerald-400 font-semibold">
            <span className="w-2.5 h-2.5 bg-emerald-500/50 rounded-sm inline-block"></span>
      ...
      <svg width={size} height={size} className="overflow-visible">
        ...
        {categories.map((cat, i) => {
          ...
          const textPt = getCoordinates(1.22, i);
          return (
            <g key={i}>
              <line
                x1={center}
        ...
        {projectedVals.map((val, i) => {
          const pt = getCoordinates(val, i);
          return <circle key={`p-${i}`} cx={pt.x} cy={pt.y} r="3" fill="#10b981" />;
        })}
